...
public class EunoiaSettingsActivity extends BaseSetupWizardActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String metricsHelpImproveEunoia =
                getString(R.string.services_help_improve_cm, os_name);
        String metricsSummary = getString(R.string.services_metrics_label,
                metricsHelpImproveEunoia, os_name, os_name);
        final SpannableStringBuilder metricsSpan = new SpannableStringBuilder(metricsSummary);
        metricsSpan.setSpan(new android.text.style.StyleSpan(android.graphics.Typeface.BOLD),
                0, metricsHelpImproveEunoia.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        if (mSupportsKeyDisabler) {
            mNavKeys.setChecked(EunoiaSettings.System.getIntForUser(getContentResolver(),
                    EunoiaSettings.System.FORCE_SHOW_NAVBAR, 0, UserHandle.USER_CURRENT) != 0);
        } else {
    ...
    protected int getLayoutResId() {
        return R.layout.setup_eunoia_settings;
    }
    ...
    private void updateDisableNavkeysOption() {
        if (mSupportsKeyDisabler) {
            final Bundle myPageBundle = mSetupWizardApp.getSettingsBundle();
            boolean enabled = EunoiaSettings.System.getIntForUser(getContentResolver(),
                    EunoiaSettings.System.FORCE_SHOW_NAVBAR, 0, UserHandle.USER_CURRENT) != 0;
    ...
    private static boolean isKeyDisablerSupported(Context context) {
        final EunoiaHardwareManager hardware = EunoiaHardwareManager.getInstance(context);
        return hardware.isSupported(EunoiaHardwareManager.FEATURE_KEY_DISABLE);
    }
}
